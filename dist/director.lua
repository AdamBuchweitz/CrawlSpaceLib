...
******************
 ...
 - 07-OCT-2010 - Ricardo Rauber - Functions loadScene and fxEnded were
								  ...
								  memory clean up;
								  Added director.directorView and effectView groups
 ...
 - 08-SEP-2011 - Lance Ulmer    - Changed director files to use the module style 
                                  found in this post "http://blog.anscamobile.com/
                                  2011/09/a-better-approach-to-external-modules/" 
                                  instead of the deprecated module() function.
...
local initViews = function()
	director.directorView:insert( currView )
	director.directorView:insert( prevView )
	director.directorView:insert( nextView )
	director.directorView:insert( protectionView )
	director.directorView:insert( popupView )
	director.directorView:insert( effectView )
end
...
local loadScene = function( moduleName, target, params )
	...
	if string.lower( target ) == "prev" then
		...
		bookBackground:addEventListener( "touch", director.moveBookPage )
...
director.getCurrBookPage = function()
...
director.getCurrBookPageNum = function()
	return currBookPage
end
...
director.getBookPageCount = function()
...
function director:newBookPages( pageList, fade )
	...
	while director.getBookPageCount() > 0 do
...
function director:changeBookPage( target )
	...
	if string.lower( target ) == "next" then
		if director.getCurrBookPageNum() + 1 < director.getBookPageCount() then
			target = director.getCurrBookPageNum() + 1
		else
			target = director.getBookPageCount()
		end
	elseif string.lower( target ) == "prev" then
		if director.getCurrBookPageNum() - 1 > 1 then
			target = director.getCurrBookPageNum() - 1
		else
			target = 1
		end
	...
	if target < 1 or 
	   target > director.getBookPageCount() or
	   target == director.getCurrBookPageNum() then
		...
		-- Moved left
		if target > director.getCurrBookPageNum() then
...
director.moveBookPage = function( event )
	...
	elseif event.phase == "ended" then
		...
		elseif currView.x > _W * limit then
			...
			director:changeBookPage( director.getCurrBookPageNum() )
		end
...
function director:goToPage( params, target, fade )
	...
	local change = function( e )
		...
		-- Last page
		elseif target == director.getBookPageCount() then
