...
return function(CSL, private, cache)
	private.random, private.floor, private.ceil, private.abs, private.sqrt, private.atan2, private.pi = math.random, math.floor, math.ceil, math.abs, math.sqrt, math.atan2, math.pi
	private.helpArr.setFillColor = 'myRect:setFillColor( hex )\n\n\tor\n\n\tmyRect:setFillColor( red, green, blue [, alpha] )'
	...
	hexToRGB = function(h, format)
		...
		if hex:len() >= 6 then
			...
			a = private.tonum(hex:sub(7, 8), 16)
	...
	private.crawlspaceFillColor = function(self,r,g,b,a)
		...
		if type(r) == "string" then
			...
			if hex:len() >= 6 then
				...
				g = private.tonum(hex:sub(3, 4), 16)
				b = private.tonum(hex:sub(5, 6), 16)
				a = private.tonum(hex:sub(7, 8), 16) or 255
			elseif hex:len() == 3 then
				r = private.tonum(hex:sub(1, 1) .. hex:sub(1, 1), 16)
				g = private.tonum(hex:sub(2, 2) .. hex:sub(2, 2), 16)
				b = private.tonum(hex:sub(3, 3) .. hex:sub(3, 3), 16)
	...
	private.injectedDisplayMethods = {}
	injectDisplayMethod = function(name, method)
		if type(method) ~= "function" then
			error("Please pass a method to inject")
		else
			private.injectedDisplayMethods[#private.injectedDisplayMethods+1] = {name, method}
		end
	end
	
	private.tranc = transition.cancel
	private.displayMethods = function( obj )
		local d = obj
		d.distanceTo = function(self,x,y) return private.ceil(private.sqrt( ((y - self.y) * (y - self.y)) + ((x - self.x) * (x - self.x)))) end
		d.angleTo = function(self,x,y) return private.ceil(private.atan2( (y - self.y), (x - self.x) ) * 180 / private.pi) + 90 end
		...
		d.fader={}
		d.fadeIn = function( self, num, callback ) private.tranc(d.fader); d.alpha=0; d.fader=transition.to(d, {alpha=1, time=num or d.fadeTime or 500, onComplete=callback}) end
		d.fadeOut = function( self, time, callback, autoRemove) d.callback = callback; if type(callback) == "boolean" then d.callback = function() display.remove(d) end elseif autoRemove then d.callback = function() callback(); display.remove(d); d=nil end end private.tranc(d.fader); d.fader=transition.to(d, {alpha=0, time=time or d.fadeTime or 500, onComplete=d.callback}) end
		...
		if d.setFillColor then d.cachedFillColor = d.setFillColor; d.setFillColor = private.crawlspaceFillColor end
		if #private.injectedDisplayMethods > 0 then
			for i,v in ipairs(private.injectedDisplayMethods) do
